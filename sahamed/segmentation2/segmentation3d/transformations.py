...
# def pad_zeros(inp: np.ndarray):#, tar: np.ndarray):
     
#     pad_size_z = int((maxdim_z - inp.shape[2])/2)
#     pad_size_y = int((maxdim_y - inp.shape[1])/2)
#     pad_size_x = int((maxdim_x - inp.shape[0])/2)

#     pad_array = [pad_size_z, pad_size_z, pad_size_y, pad_size_y, pad_size_x, pad_size_x]
#     inp_padded = F.pad(inp, pad_array, mode='constant', value=0)
#     # tar_padded = F.pad(tar, pad_array, mode='constant', value=0)
...
def pt_preprocess_1(inp, trunc=45):
    inp[inp<0] = 0
    inp[inp>trunc]=trunc
    inp_trunc=inp/trunc
    return inp_trunc
...
def pt_preprocess_2(inp, patient_mean, patient_std):
    inp_norm = (inp - patient_mean)/(patient_std + 1e-8)
    return inp_norm


def ct_preprocess_1(inp):
    lower_bound = -1024
    upper_bound = 1024
