...
#%%
dataloader_train = DataLoader(dataset=dataset_train, batch_size=256, shuffle=True, pin_memory=True, num_workers=24)
dataloader_valid = DataLoader(dataset=dataset_valid, batch_size=256, shuffle=True, pin_memory=True, num_workers=24)
...
model = UNet(spatial_dims=2,\
            ...
            strides=(2, 2, 2, 2, 2)).to(config.MAIN_DEVICE)
...
criterion = GeneralizedDiceLoss(include_background=False,\
                                to_onehot_y=False,\
                                softmax=True).to(config.MAIN_DEVICE)
# %%
# trainer
import time 
starttime = time.time()
